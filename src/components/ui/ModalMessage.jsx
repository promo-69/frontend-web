import React from 'react'
import { LoginIcon, IconError } from './IconosProyect'

const ModalMessage = ({ type = 'success', message, onClose }) => {
  const isSuccess = type === 'success'

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="bg-[#231640] p-10 rounded-xl flex flex-col items-center justify-center border border-[#7B1A82] shadow-2xl max-w-sm w-full mx-4"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Ícono dinámico */}
        <div
          className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 shadow-lg
          ${isSuccess ? 'bg-[#D9982F]' : 'bg-red-500'}`}
        >
          {isSuccess ? (
            <LoginIcon className="w-12 h-12 text-[#231640]" />
          ) : (
            <IconError className="w-12 h-12 text-white" />
          )}
        </div>

        {/* Mensaje */}
        <p className="text-white text-2xl font-bold text-center leading-tight mb-2">
          {message}
        </p>

        {/* Botón */}
        <button
          type="button"
          onClick={onClose}
          className={`mt-8 w-full font-bold rounded-full py-2 text-[13px] uppercase transition-all active:scale-95 shadow-lg
            ${isSuccess ? 'bg-[#D9982F] text-[#231640]' : 'bg-red-500 text-white'}`}
        >
          Cerrar
        </button>
      </div>
    </div>
  )
}

export default ModalMessage
