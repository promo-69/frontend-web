...
function FormEditProfile({ userData, step, setStep, onSave, loading }) {
  ...
  const validate = () => {
    ...
    const hasNameChanged = name.trim() !== (userData.name || '').trim();
  ...
  return (
    <div className="w-full max-w-[360px] bg-white/5 p-5 rounded-3xl backdrop-blur-md border border-white/10 shadow-xl">
      <form className="space-y-4 text-white" onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-3 opacity-80">
          <div className="border-b border-gray-500 pb-1">
            ...
            {isEditing && errors.name && (
              <span className="text-[9px] text-red-400 italic">{errors.name}</span>
            )}
          </div>
          <div className="border-b border-gray-500 pb-1">
            <label className="text-[9px] font-bold text-gray-400 uppercase">
            ...
            {isEditing && errors.lastname && (
              <span className="text-[9px] text-red-400 italic">{errors.lastname}</span>
            )}
          </div>
          <div className="col-span-2 border-b border-gray-500 pb-1">
            <label className="text-[9px] font-bold text-gray-400 uppercase">
              Cédula
            </label>
        ...
        {/* Teléfono*/}
        <div
          className={`border-b flex flex-col ${isEditing ? 'border-[#D9982F]' : 'border-gray-500'}`}
        >
          <label
            htmlFor="phone-field"
            className="text-[9px] font-bold text-gray-400 uppercase cursor-pointer"
          ...
          <div className="flex items-center gap-2 py-1">
            ...
            >
              <option value="+58" className="bg-[#231640]">
                VE +58
              </option>
              <option value="+57" className="bg-[#231640]">
                CO +57
              </option>
            </select>
            <input
