...
export default function ForU() {
  ...
  const handleGenreBannerClick = () => {
    if (!isAuthenticated) {
      setModalConfig({
        ...
        message: 'Para poder seleccionar tus géneros cinematográficos preferidos y armar una cartelera personalizada, necesitas acceder a tu cuenta de Cineflix.'
      });
  ...
  const handleRoomsBannerClick = () => {
    if (!isAuthenticated) {
      setModalConfig({
        title: '¿Reservar Sala?',
        message: 'Para consultar la disponibilidad, tarifas y realizar el alquiler de nuestras salas privadas de cine, debes iniciar sesión primero.'
      });
      setShowLoginModal(true);
    } else {
      navigate('/room-rent'); 
